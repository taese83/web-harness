...
- **디자인 근거 없이 화면의 조건을 지어내지 않는다.** 빈 상태·오류·권한 없음처럼 조건에 따라
  ...
  **멈추지 않는 두 경우**: ⓐ 정보 위계 표에 내용이 있으면 그것이 근거다 — 그리는 방식은
  스스로 정하고 형상 규율대로 한 줄 보고한다. ⓑ `_workspace/01_plan/decision-log.md`(또는 분할된 `decision-log/`)에 그
  조건(또는 "이 프로젝트의 화면 조건 전부")을 **인수한 `PC-NNN` 엔트리**가 있으면, 사용자가
  이미 "구현이 그 자리에서 정하는 것"을 받아들인 것이다. 그때는 정하고 무엇을 정했는지
  보고한다 — **인수 기록을 무시하고 멈추면 사용자가 고른 경로가 완결되지 않는다**
