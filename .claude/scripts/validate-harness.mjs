...
import {inspectShapeCatalog} from './validate-shape-checks.mjs'
...
if (!templateSource.includes('failOnFlakyTests: Boolean(process.env.CI)')) fail('project template Playwright config does not fail flaky tests in CI')
// axe 기본 실행은 WCAG 2.2 target-size(2.5.8)를 끈다 — 켜지 않으면 2.2 점검 주장이 증명되지 않는다.
if (!templateSource.includes("'target-size': {enabled: true}")) fail('project template axe scan does not enable target-size')
// 라우터 안 오류는 App 경계에 오지 않는다 — 라우트 ErrorBoundary가 없으면 기본 화면이 stack을 노출한다.
if (!templateSource.includes('## ROUTE_ERROR_BOUNDARY') || !templateSource.includes('ErrorBoundary: RouteErrorBoundary')) fail('project template routes lack a route ErrorBoundary')
...
validateWorkflowsAndEvals({claudeDirectory, repositoryRoot, pass, fail}); validateAnalyticsBuilder({repositoryRoot, read, pass, fail}); validateVisualDesign({repositoryRoot, read, pass, fail}); validatePlanningFacilitation({repositoryRoot, read, pass, fail}); validateAdapterHygiene({repositoryRoot, pass, fail});
{
  const shapeCatalogErrors = inspectShapeCatalog();
  for (const message of shapeCatalogErrors) fail(message);
