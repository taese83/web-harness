...
export const validateAgentBoundaries = ({
  ...
  const cases = [
    ['owned path', 'test-writer', join(repositoryRoot, 'src/entities/item/__tests__/a.spec.ts'), 0],
    ...
    ['other agent nvmrc stays blocked', 'test-writer', join(repositoryRoot, '.nvmrc'), 2],
    ['environment scaffolder root claude-md marker', 'environment-scaffolder', join(repositoryRoot, 'CLAUDE.md'), 0],
    ['other agent claude-md stays blocked', 'test-writer', join(repositoryRoot, 'CLAUDE.md'), 2],
    ...
    ['another agent path', 'test-writer', join(repositoryRoot, 'src/app/App.tsx'), 2],
    ['outside-project path', 'test-writer', resolve(repositoryRoot, '../outside.ts'), 2],
    ...
    ['realtime builder chart UI', 'test-writer', join(repositoryRoot, 'src/features/live-mode/ui/LiveBadge.tsx'), 2],
    ['foundation builder realtime scope', 'shared-foundation-builder', join(repositoryRoot, 'src/shared/realtime/transport.ts'), 2],
    ['form builder live-mode scope', 'test-writer', join(repositoryRoot, 'src/features/live-mode/model/store.ts'), 2],
    ['form builder live-mode api scope', 'test-writer', join(repositoryRoot, 'src/features/live-mode/api/connect.ts'), 2],
    ...
    ['model gateway source', 'model-gateway-builder', join(repositoryRoot, 'packages/model-gateway/src/index.ts'), 0],
    ['model gateway builder web source', 'model-gateway-builder', join(repositoryRoot, 'apps/web/src/features/chat/api.ts'), 2],
    ['Next config scaffold', 'next-app-scaffolder', join(repositoryRoot, 'next.config.ts'), 0],
    ['Next config scaffold route source', 'next-app-scaffolder', join(repositoryRoot, 'src/app/page.tsx'), 2],
    ...
    ['client domain state builder UI source', 'test-writer', join(repositoryRoot, 'src/widgets/board/ui/Board.tsx'), 2],
    ...
    ['ingestion pipeline source', 'external-data-pipeline-builder', join(repositoryRoot, 'packages/ingestion/src/normalize.ts'), 0],
    ['ingestion pipeline builder UI source', 'external-data-pipeline-builder', join(repositoryRoot, 'src/widgets/dashboard/ui/Dashboard.tsx'), 2],
    ...
    ['visual test output', 'visual-test-writer', join(repositoryRoot, 'e2e/home.visual.spec.ts'), 0],
    ...
    ['db migration file', 'db-migration-writer', join(repositoryRoot, 'migrations/001_initial_schema.sql'), 0],
    ['db migration nested app location', 'db-migration-writer', join(repositoryRoot, 'client/migrations/002_add_profiles.sql'), 0],
    ['db migration runner script', 'db-migration-writer', join(repositoryRoot, 'scripts/migrate.ts'), 0],
    ['db migration writer env file', 'db-migration-writer', join(repositoryRoot, '.env.local'), 2],
    ['db migration writer app source', 'db-migration-writer', join(repositoryRoot, 'src/app/App.tsx'), 2],
    ['seo spec output', 'seo-meta-builder', join(repositoryRoot, '_workspace/02_design/seo-spec.md'), 0],
    ['seo robots file', 'seo-meta-builder', join(repositoryRoot, 'public/robots.txt'), 0],
    ['seo sitemap file', 'seo-meta-builder', join(repositoryRoot, 'public/sitemap.xml'), 0],
    ['seo shared util', 'seo-meta-builder', join(repositoryRoot, 'src/shared/seo/meta.ts'), 0],
    ['seo builder index html', 'seo-meta-builder', join(repositoryRoot, 'index.html'), 2],
    ['seo builder next app source', 'seo-meta-builder', join(repositoryRoot, 'src/app/page.tsx'), 2],
    ...
    ['i18n spec output', 'i18n-builder', join(repositoryRoot, '_workspace/02_design/i18n-spec.md'), 0],
    ['i18n catalog file', 'i18n-builder', join(repositoryRoot, 'src/shared/lang/ko/common.json'), 0],
    ['i18n builder route source', 'i18n-builder', join(repositoryRoot, 'src/app/routes/Routes.tsx'), 2],
    ['observability spec output', 'web-observability-builder', join(repositoryRoot, '_workspace/02_design/observability-spec.md'), 0],
    ['observability init module', 'web-observability-builder', join(repositoryRoot, 'src/shared/observability/init.ts'), 0],
    ['observability builder deploy workflow', 'web-observability-builder', join(repositoryRoot, '.github/workflows/deploy.yml'), 2],
  ]
