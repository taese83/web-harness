#!/usr/bin/env node
// test-work-close.mjs — 자동 닫기(대상 프로젝트 CI에서 도는 자산)를 **실제 프로세스**로 돌린다.
//
// 가짜 `gh`를 PATH 앞에 둬 호출을 기록한다 — 실 GitHub은 NOT_RUN이다. 고정하는 사실:
//   (1) 근거는 이슈의 연결 기록 — 가장 최근 연결이 이 PR·이 머지 base인 하네스 작업 이슈만 닫는다(T18)
//   (2) PR 본문의 `#N`은 후보일 뿐 — 하네스 작업 이슈가 아니거나 연결 기록이 없거나 다른 PR·다른 base면 닫지 않는다
//   (3) 이미 닫힌 이슈는 다시 닫지 않고, 한 건 실패로 나머지를 버리지 않는다
//   (4) 옛 판본 사본은 설치됨으로 세지 않고 알린다(T25) — 덮어쓰지는 않는다
import assert from 'node:assert/strict'
import test from 'node:test'
import {chmodSync, mkdirSync, mkdtempSync, readFileSync, rmSync, writeFileSync} from 'node:fs'
import {join} from 'node:path'
import {tmpdir} from 'node:os'
import {spawnSync} from 'node:child_process'
import {installTicketCloseAssets, planTicketCloseInstall, TICKET_CLOSE_VERSION_MARKER} from './ticket/cli.mjs'
import {renderLinkRecord} from './ticket/work-records.mjs'
import {buildWorkMarker} from './ticket/work-refs.mjs'
import {checkTicketAssets} from './validate-development-readiness.mjs'

const repo = new URL('../..', import.meta.url).pathname
const SCRIPT = join(repo, '.claude/skills/team-flow/assets/close-merged-tickets.mjs')
const PR = 'https://github.com/acme/web/pull/42'
const WORK_BODY = `작업 본문\n\n${buildWorkMarker({planId: '22222222-2222-4222-8222-222222222222', workId: 'WORK-00000001-0000-4000-8000-000000000001',
  featureIds: ['FEAT-001'], testCaseIds: [], planDigest: 'a'.repeat(64)})}`
const link = (createdAt, {pr = PR, base = 'feature/members', association = 'MEMBER'} = {}) => ({author: {login: 'dev'}, authorAssociation: association, createdAt, body: renderLinkRecord({prUrl: pr, baseRef: base})})
const issue = (comments, {state = 'OPEN', body = WORK_BODY} = {}) => ({state, body, comments})

function run(issues, {base = 'feature/members', prBody = Object.keys(issues).map(number => `Closes #${number}`).join('\n'), failClose = null} = {}) {
  const root = mkdtempSync(join(tmpdir(), 'wh-close-'))
  try {
    const bin = join(root, 'bin')
    mkdirSync(bin)
    mkdirSync(join(root, 'issues'))
    for (const [number, value] of Object.entries(issues)) writeFileSync(join(root, 'issues', `${number}.json`), JSON.stringify(value))
    const calls = join(root, 'gh-calls.log')
    writeFileSync(join(bin, 'gh'), `#!/bin/sh\necho "$@" >> "${calls}"\n`
      + `if [ "$1" = "issue" ] && [ "$2" = "view" ]; then if [ -f "${root}/issues/$3.json" ]; then cat "${root}/issues/$3.json"; exit 0; fi; echo "not found" >&2; exit 1; fi\n`
      + (failClose ? `if [ "$1" = "issue" ] && [ "$2" = "close" ] && [ "$3" = "${failClose}" ]; then echo "HTTP 403" >&2; exit 1; fi\n` : ''))
    chmodSync(join(bin, 'gh'), 0o755)
    const result = spawnSync(process.execPath, [SCRIPT], {cwd: root, encoding: 'utf8', timeout: 30000,
      env: {PATH: `${bin}:${process.env.PATH}`, TICKET_REPO: 'acme/web', TICKET_PR_URL: PR, TICKET_BASE_REF: base, TICKET_PR_BODY: prBody}})
    let ghCalls = []
    try { ghCalls = readFileSync(calls, 'utf8').trim().split('\n').filter(Boolean) } catch { /* 호출 없음 */ }
    return {status: result.status, stdout: result.stdout, ghCalls, closes: ghCalls.filter(call => call.startsWith('issue close'))}
  } finally { rmSync(root, {recursive: true, force: true}) }
}

test('(1) 가장 최근 연결이 이 PR·이 base인 하네스 작업 이슈만 닫는다 — 근거를 코멘트로 남긴다', () => {
  const result = run({12: issue([link('2026-09-14T00:00:00Z')])})
  assert.equal(result.status, 0, result.stdout)
  assert.ok(result.closes.some(call => call.startsWith('issue close 12 --repo acme/web --comment')), result.ghCalls.join('\n'))
  assert.match(result.stdout, /closed #12/)
  assert.match(result.stdout, /done: 1\/1 closed/)
})

test('(2) 본문의 #N은 후보일 뿐 — 작업 이슈가 아니거나 연결 기록이 없거나 다른 PR·다른 base면 닫지 않는다', () => {
  const otherBase = run({12: issue([link('2026-09-14T00:00:00Z')])}, {base: 'main'})
  assert.equal(otherBase.closes.length, 0, '다른 브랜치 머지로 작업을 닫았다')
  assert.match(otherBase.stdout, /기대 base feature\/members ≠ 머지 base main/)
  // 남의 이슈 번호를 본문에 적어도 연결 기록이 없으면 닫지 않는다.
  assert.equal(run({12: issue([])}).closes.length, 0, '연결 기록 없는 이슈를 닫았다')
  assert.equal(run({12: issue([link('2026-09-14T00:00:00Z')], {body: '사람이 쓴 이슈'})}).closes.length, 0, '하네스 작업 이슈가 아닌 것을 닫았다')
  // 집계 표지가 있으면 작업 표지가 함께 있어도(종류가 모호하다) 닫지 않는다.
  const aggregate = `${WORK_BODY}\n<!-- web-harness:aggregate plan=x feat=FEAT-001 -->`
  assert.equal(run({12: issue([link('2026-09-14T00:00:00Z')], {body: aggregate})}).closes.length, 0, '집계 티켓을 닫았다')
  // 이 PR에 연결됐다가 다른 PR로 다시 연결됐다 — 가장 최근 연결이 정본이다(코멘트 순서가 아니라 시각).
  const relinked = run({12: issue([link('2026-09-15T00:00:00Z', {pr: 'https://github.com/acme/web/pull/99'}), link('2026-09-14T00:00:00Z')])})
  assert.equal(relinked.closes.length, 0, '다른 PR로 옮겨 간 작업을 닫았다')
  // 공개 저장소에서 저장소 밖 사람이 남긴 연결 기록은 세지 않는다.
  assert.equal(run({12: issue([link('2026-09-14T00:00:00Z', {association: 'NONE'})])}).closes.length, 0, '외부인이 남긴 연결 기록으로 닫았다')
  // 시각 없는 기록은 세지 않는다.
  assert.equal(run({12: issue([{...link('2026-09-14T00:00:00Z'), createdAt: null}])}).closes.length, 0)
  const noCandidates = run({}, {prBody: '설명만 있다'})
  assert.equal(noCandidates.ghCalls.length, 0)
  assert.match(noCandidates.stdout, /닫을 후보가 없다/)
})

test('(3) 이미 닫힌 이슈는 다시 닫지 않고, 한 건을 닫지 못해도 나머지를 처리해 exit 1로 알린다', () => {
  const closed = run({12: issue([link('2026-09-14T00:00:00Z')], {state: 'CLOSED'})})
  assert.equal(closed.closes.length, 0)
  assert.match(closed.stdout, /이미 CLOSED/)
  const result = run({12: issue([link('2026-09-14T00:00:00Z')]), 13: issue([link('2026-09-14T00:00:00Z')])}, {failClose: '12'})
  assert.equal(result.status, 1)
  assert.ok(result.closes.some(call => call.startsWith('issue close 13')), '첫 실패에서 나머지를 버렸다')
  assert.match(result.stdout, /failed 1: #12/)
})

test('(4) 옛 청구 원장 기반 사본은 설치됨으로 세지 않고 알린다 — 덮어쓰지 않는다(T25)', () => {
  const root = mkdtempSync(join(tmpdir(), 'wh-close-install-'))
  try {
    mkdirSync(join(root, '_workspace/03_dev'), {recursive: true})
    writeFileSync(join(root, '_workspace/03_dev/work-item-events.jsonl'), '')
    const fresh = planTicketCloseInstall(root)
    assert.deepEqual(installTicketCloseAssets(root, fresh).length, 2)
    assert.ok(readFileSync(join(root, '.github/scripts/close-merged-tickets.mjs'), 'utf8').includes(TICKET_CLOSE_VERSION_MARKER))
    assert.equal(checkTicketAssets(root).state, 'PASS')
    // 옛 사본(v1 — 판본 표지 없음)으로 바꿔 놓는다.
    const old = '// 머지된 PR에 묶인 티켓 이슈를 닫는다.\nconst LEDGER = \'_workspace/03_dev/identity-ledger.jsonl\'\n'
    writeFileSync(join(root, '.github/scripts/close-merged-tickets.mjs'), old)
    const plan = planTicketCloseInstall(root)
    assert.deepEqual(plan.outdated, ['.github/scripts/close-merged-tickets.mjs'])
    assert.deepEqual(plan.install, [])
    const verdict = checkTicketAssets(root, {install: true})
    assert.equal(verdict.state, 'FAIL', '옛 사본을 설치됨으로 셌다')
    assert.equal(readFileSync(join(root, '.github/scripts/close-merged-tickets.mjs'), 'utf8'), old, '손봤을 수 있는 사본을 덮었다')
  } finally { rmSync(root, {recursive: true, force: true}) }
})
