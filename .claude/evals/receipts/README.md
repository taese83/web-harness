...
`<scenario-id>/<run-id>.json`. 배포본 플러그인 평가(`run-plugin-evals.mjs`)의 receipt는 `plugin/<시각>.json`이다. 규약과 커밋 기준은 `.claude/evals/README.md` "실행 receipt" 절이
정본이다. transcript·fixture는 커밋하지 않는다(`eval-runs/`는 VCS 제외).

현재 receipt **9건** — `complete-harness-packaging`(2026-08-27)과 저장소 모드 회귀 묶음 8건(0.45.0까지 릴리스 전 실행 — 이후 릴리스 회귀는 `plugin/`). 규약 선언에서 실행 증거로 넘어온
