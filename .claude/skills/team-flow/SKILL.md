...
남긴다) · 소유 TC 인용 · `checks` 대상 실재와 **픽업 뒤 변화** · **기대 base**(PR에서 읽거나 `--base`, 모르면 막는다) · **PR 제목의 티켓 키**
(`[키] …` — 머지된 PR을 제목으로 찾는다, 키가 없으면 막고 제목을 못 읽으면 알린다) · 멱등. 미충족은 막고 `--accept-incomplete`로 넘기면 그 사실이 기록에 남는다.
...
완료는 기록하지 않는다 — 보드·픽업이 기대 base에 **머지된 PR을 제목의 티켓 키로** 찾고(머지된 커밋·트래커의 끝남도) 그때그때 읽는다.
