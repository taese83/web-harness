...
| "이 FEAT를 그 티켓에 묶어줘", "티켓에 마커 붙여줘" | `bind <FEAT> <티켓키>` |

**역방향 흐름**(사람이 트래커에 직접 쓴 티켓이 입구일 때): `intake` → `source-artifact-ingestor`가
정규화하고 분류를 정한다 → `feature-planner`가 FEAT·TC를 만든다 → `bind` → 그때부터 `pickup`이
그 티켓을 알아본다. **`intake`는 분류도 요구사항도 판정하지 않는다** — 근거만 싣는다.
