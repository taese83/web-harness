...
> **상태(2026-08-28)**: 브라운필드의 승인 표면이던 **라이브 델타를 제거했다** — 실행 중인 앱에
> 프록시로 오버레이를 주입해 승인을 성립시키던 기제이며, 그 결합이 CSP·SSR·Shadow DOM 미실증과
> 신원 대조·anchorReceipt 같은 부수 기제를 끌고 왔다. 승인은 **프리뷰 한 곳**으로 모은다.
> **그 결과 지금 브라운필드에는 기획 확인 표면이 없다.** 프리뷰가 없는 기존 서비스의 바탕을
> DOM 스냅샷으로 만드는 작업이 예정돼 있으며, 그것이 들어오면 이 계약을 "승인은 언제나 프리뷰,
> 바탕만 다르다"로 다시 쓴다. **그때까지 이 문서의 태생별 표면 배정·전환 규칙은 유효하지 않다.**
