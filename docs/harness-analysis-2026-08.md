...
> **2026-08-27 후속**: AI 앱 생성 표면을 전면 제거했다(에이전트 46 · 스킬 24 · 진입 비용 63,653B).
> 이 문서의 수치는 2026-08-26 시점 단면이다. 현행 수치는 README, 근거는 `protected-core.md` §4의
> "AI 표면 전면 제거" 행을 본다.
...
**어댑터 삭제가 미완이다 — 그리고 이 문단의 진단이 틀렸다(2026-08-27 정정).** 막고 있던 것은
1,422줄 스캐폴딩이 아니었다. 교체본 두 개가 **소비처 0으로 배선된 적이 없었고**, `next-app-fullstack`은
도출과 등가가 아니며(선언 32 tasks vs 도출 12, 교집합 7), 어댑터의 41%(`detection`·`trust`·
`deployment*`)에는 교체본이 아예 없다. `trust`는 안전 하한이라 이관 대상도 아니다. 지금은 Vite 2종의
등가를 CI에 결박한 단계이며, 삭제는 격리 CI 재실행이 가능할 때다 — 근거는 §4 "어댑터 삭제 — 전제가
틀렸다" 행.
