...
- **이슈 브랜치 스탬프**(라벨 `branch:<name>` + 왕복 마커 `branch=` 필드)가 repo 전역 레지스트리 —
  `branch:*` 라벨 목록이 곧 공식 작업 브랜치 명단(트래커만으로 크로스-머신 집계).
...
- 미청구 기획 브랜치는 오버뷰에 **안 보이는 게 정직**(청구 = 팀 공표 행위). 청구된 브랜치가
  origin에서 사라지면 "브랜치 소실" 경고로 정직 표기(침묵 실종 금지).

**4-2. 콘솔 Development 2단 구조.** 로컬 파일은 브랜치-스코프지만 크로스-브랜치 창이 둘 있다:
트래커(브랜치 스탬프)와 `git show origin/<br>:<path>`(체크아웃 없이 타 브랜치의 plan·원장 읽기).
- **Level 1 오버뷰**: 브랜치 카드 목록(레지스트리 = `branch:*` 라벨) — 기획 제목·티켓 분포(진행/
  미선택/blocked/머지)·병목 뱃지. 브랜치는 "한정"이 아니라 **1차 그룹 축**이고 집계가 그 위에 얹힘.
