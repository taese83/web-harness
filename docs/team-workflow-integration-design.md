...
- **콘솔 v1 구현 결정(2026-08-24, 증분 3 — 이후 축 정정)**: Development › Work flow pane은
  트래커(gh) 미연동(배정·이슈 상태 미상, claimed = "청구됨"만), 자동 fetch 안 함(스냅샷 기준
  표기), 브랜치 발견은 원장 자기-일치. **티켓 행의 기준은 push된 형상(origin)이다**(사용자
  확정: 개발자는 push된 형상 기반으로 확인, 로컬에서 바로 발행 불가) — 미발급 행은
  `unclaimed`(push됨·발행 대기, **청구 가능한 유일한 미발급**) / `local-new`(미푸시 신규) /
  `local-modified`(미푸시 수정)로 3분할하고, 발급 행의 로컬 편집은 `localDrift` 배지로 표기,
  STALE은 push된 상류 변경만 뜻한다. 현재 브랜치는 §4-1 예외로 미청구여도 카드가 보이되 기준
  축은 동일하게 origin이고 로컬은 오버레이(차이 표기)다. 상태 그룹(전체/발급됨/발행 대기/로컬
  작업) **탭 필터** 제공, 상태 안내는 행 반복 없이 카드당 한 줄. 표 형식/구세대 계획은 "형식
  미지원"으로 정직 표기.
