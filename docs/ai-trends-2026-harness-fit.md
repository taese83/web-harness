...
| 11 | 자체 에이전트의 웹 인입 격리 | 실측(2026-08-31): 계약 전달 지시가 `web-orchestrator/SKILL.md`에만 있었고 `/wh`의 나머지 세 레인 정본에는 0건이었다. **같은 날 부분 해소** — 규칙을 웹 인입 4종의 에이전트 계약으로 내리고 미참조를 `validate-agent-boundaries`가 FAIL시킨다. 준수 자체는 여전히 미검증(§4 등록) | **공백 · 부분 해소** |
...
**2026-08-31 부분 해소.** 규칙을 레인 문서가 아니라 웹 인입 4종의 **에이전트 계약**에 넣고,
`validate-agent-boundaries`가 에이전트 디렉터리에서 도출한 목록으로 미참조를 FAIL시킨다(무장 확인:
참조 1건 제거 시 FAIL 발화 실측). **닫힌 것은 "지시가 레인에 따라 사라지는 문제"까지다** — 발췌
길이·`INJECTION_SUSPECT` 기록·1차 출처 대조의 **준수 여부는 여전히 기계로 확인되지 않으며**,
이 층에는 티켓 경계의 `scanUntrustedBody` 같은 탐지기가 없다. 한계는 §4에 등록했다.
...
| 우선 | 조치 | 성격 | 건드리는 불변식 |
|---|---|---|---|
| 1 | ~~**격리 규칙을 레인이 아니라 에이전트 계약에 둔다**~~ — **2026-08-31 시행(부분 해소, 준수 검증은 미해결)** — `untrusted-content-quarantine.md` 참조 + 인용 발췌 규칙을 웹 인입 4종에 넣고, 미배선을 검사로 고정. 레인 문서를 늘리지 않는 이유: 갈라진 것이 원인이고, 에이전트 파일은 스폰당 비용이라 진입 고정비를 건드리지 않는다 | 결손 배선 복구(신규 계약 아님) | I6 하한, I5(정규식 프록시 한계 표기), I4 무증가 |
