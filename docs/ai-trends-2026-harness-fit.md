...
| 11 | 자체 에이전트의 웹 인입 격리 | 실측(2026-08-31): WebSearch/WebFetch를 가진 4개 에이전트(`requirements-analyst`·`tech-advisor`·`ux-researcher`·`source-artifact-ingestor`) 중 **`untrusted-content-quarantine.md`를 참조하는 것은 0개** | **공백** |
...
인바운드 격리는 **티켓 경계에서만** 기계화돼 있다(`scanUntrustedBody`). 그러나 리서치 계열 4개
에이전트가 WebSearch/WebFetch로 외부 문서를 읽고, 그 결과가 기획서·기술 선택으로 흘러간다.
`untrusted-content-quarantine.md`는 그 경로를 규정하지만 **해당 에이전트 어느 것도 그 계약을
참조하지 않는다**(실측 0건). 웹 콘텐츠 인젝션 페이로드가 4개월 새 32% 늘었다는 관측과 겹치면,
이것은 문서화된 계약이 **가장 노출된 경로에 배선되지 않은** 전형적인 마커-사슬 결손이다 — §4에
이미 여러 번 등록된 실패 클래스와 같은 모양이다.
...
| 우선 | 조치 | 성격 | 건드리는 불변식 |
|---|---|---|---|
| 1 | **웹 인입 4개 에이전트에 격리 계약 배선** — `untrusted-content-quarantine.md` 참조 + 인용 발췌 규칙을 에이전트 계약에 넣고, 미배선을 검사로 고정 | 결손 배선 복구(신규 계약 아님) | I6 하한, I5(정규식 프록시 한계 표기) |
