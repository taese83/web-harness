...
| 마커 무결성 카운트 | `validate-marker-integrity.mjs`는 등록 마커의 **문자열 출현 수**만 센다(current<baseline→FAIL) — 산문·주석에 decoy로 같은 문자열을 삽입하면 실제 계약 열(INDEX 소비자 열)을 지워도 카운트가 유지돼 통과한다. 즉 손실은 잡지만 **의미 보존은 미보장** | — | 마커 탈잠금(marker-delock-plan §5)의 **임시 안전망**으로 명시 도입(2026-08-18 리뷰에서 등록) — 앵커화(§5-3) 후 pattern을 중립 앵커로 교체하고 validate-artifact-sharding의 소비자-열 검사를 WARN→FAIL로 승격해 의미 검증으로 대체 예정. baseline 갱신(--update)은 before/after diff를 출력해 리뷰 가시성 확보 |
