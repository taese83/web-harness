...
| UI_LANE 산문 결정 (tier a) | `UI_LANE`은 tech-stack.md 기록 + 빌더 산문 준수일 뿐 — **방출된 라이브러리가 선택과 일치하는지 보는 기계 검사가 없다**. 특히 `project-init/templates.md`는 레인 무관하게 **무조건 MUI를 스캐폴드**한다(package.json deps·APP_TSX·ERROR_FALLBACK·APP_THEME) — `tailwind-shadcn` 선택 그린필드의 초기 스캐폴드에 MUI 잔재가 남아 preflight×CssBaseline 안티패턴을 자가 재현할 수 있다. vendored 프리미티브의 a11y 배선 삭제("단순화하며 aria 제거")를 잡는 기계 검사도 부재(jsx-a11y lint는 구조적 배선 삭제를 못 잡음) | 2026-08-18 리뷰 실측: templates.md 미배선 + checklist의 "MUI 컨벤션" 잔존 문구 | **tier (b)에서 해소 예정**: 레인 불일치 validator(uiLane vs src/ import 대조) + 레인별 템플릿 섹션 + code-reviewer vendored-a11y 정적 검사 + 브라운필드 no-flip eval. 그 전까지 tailwind-shadcn 그린필드는 스캐폴드 직후 MUI 의존 제거가 수동 필요함을 계약이 공시(lib-catalog §UI) |
