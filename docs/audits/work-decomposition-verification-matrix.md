...
| T03 | 공통 기반을 FEAT 둘이 사용 | PASS | U + `P`「T43·T48」 — 계획상 WORK 하나, 발행도 생성 1건(소비 FEAT는 본문 「참고」·마커에 전부) |
...
| T43 | FEAT 여러 개가 새 공통 계약 | PASS | U + `P`「T43·T48」 — 공유 작업의 생성 호출 1건, 소비 FEAT는 본문 「참고」와 마커에 전부(FEAT 라벨은 2026-09-15 사용자 결정으로 없앴다 — 라벨은 역할·팀뿐). `V`가 두 트래커의 **실 필드 빌더**가 호출자 라벨을 보존하고 마커를 보이지 않는 곳에 두는지 잰다 |
...
| T48 | 공유 WORK의 관계 표현 부족 | PASS | `V` + `P`「T43·T48」 — 발행 경로에서 FEAT별 복제 없음(생성 1건), 관계는 부모를 준 경우에만 걸고 적용 여부를 원장에 그대로 남긴다 |
...
티켓 모양 재설계(2026-09-15 사용자 결정 — 역할 라벨·개발자용 본문·보이지 않는 마커·AI 맥락 첨부·사람 편집 반영): 회귀 `test-work-ticket-doc`·`P`「T43·T48」「T47」·`K` 사람 편집·e2e(Jira provider)와 seed 23건(+앵커 갱신 6 · 은퇴 2)으로 결박, 실 GitHub(#1~#4 이관)·Jira(AOA-9~12)에서 이관 동기화·편집 반영·계획 항목 삭제 차단·보존형 동기화를 확인했다(receipt `docs/audits/receipts/2026-09-15-work-ticket-shape.json`).
