...
| 호스트 payload가 정하는 이동에 스킴 allowlist 없음, 위임 근거 미결박, 매칭 실패 폴백 ★ | security-reviewer (`webview-host-contract.md` §3 — `code-reviewer` 6의 확장이라 알림이 아닌 FAIL) |
...
| 호스트 경계(파사드·능력 축 3종·복귀·설정 동기화) ★ — 값은 계약이 아니라 프로젝트가 소유 | `webview-host-contract.md` |
...
실제 서비스에서 얻은 관측(실사고·환경 제약·호스트 동작)은 규칙이 되기 전에 **지식**이다.
지식을 규칙과 같은 자리에 두면 규칙이 특정 서비스에 묶이고(I3), 반대로 버리면 다음 팀이 같은 것을
다시 겪는다. 성질로 갈라 각자의 집에 보낸다.

| 성질 | 무엇인가 | 집 | 수명 |
|---|---|---|---|
| **플랫폼 사실** | 런타임·브라우저·OS가 그렇게 동작한다 (호스트·서비스 무관) | 해당 구현 계약의 본문 | 길다 — 플랫폼이 바뀔 때만 |
| **패턴** | 이 제약을 만나면 이렇게 푼다 (값은 프로젝트마다 다름) | 같은 계약, 규칙 형태로 | 길다 |
| **호스트 사실** | 이 호스트의 이 버전이 이렇게 동작한다 (이름·버전·수치) | **프로젝트 산출물** — 계약에 넣지 않는다 | 짧다 — 호스트 릴리스마다 |

**판별 질문은 하나다 — 호스트 팀이 릴리스로 바꿀 수 있는가. 그러면 호스트 사실이다.** 리사이즈 모드·권한 동작처럼 플랫폼처럼 보이는 것도 호스트가 설정으로 정하면 그쪽이다.
| **프로젝트 사실** | 우리 백엔드·우리 팀의 사정 | 코드 주석 또는 기존 산출물 | — |

- **호스트 사실을 계약에 쓰지 않는다.** 계약이 특정 호스트 이름·버전·환산표를 담으면 그 계약은
  한 조직 전용이 된다(I3). 계약은 *"무엇을 결정해야 하는가"*를 주고, *"그 값이 무엇인가"*는
  프로젝트가 소유한다. 조직이 여러 프로젝트에서 값을 공유하는 방법은 조직이 정한다.
- **한 관측이 둘로 갈리는 것이 정상이다.** "호스트 폰트 배율을 경계에서 한 번 읽고 클램프한다"는
  패턴이고, "iOS 시스템 배율 13단계 환산표"는 호스트 사실이다. 섞어 적으면 둘 다 못 쓴다.
- **호스트 사실에는 결박이 필요하다.** 관측 시점과 적용 버전 범위가 없으면 호스트가 다음 판을 내는
  순간 거짓이 된다 — 형식은 `docs/audits/receipts/`를 따른다. 근거를 못 대는 관측은 적지 않는다.
- `operational-gotchas.md`는 **하네스를 운영할 때의 함정**이다. 생성 앱이 만나는 환경 사실은
  그쪽이 아니라 해당 구현 계약으로 간다.
