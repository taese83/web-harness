...
기계 검증되는 숫자다 — 어긋나면 빌드가 실패한다.

- **사람이 읽는 것**: 이 README와 [docs/quickstart.md](docs/quickstart.md)가 전부다. 110여 개 계약
  문서는 **에이전트가 시점 로드**하지, 사람이 읽는 대상이 아니다.
- **오케스트레이터 1회 실행의 고정 계약 로드**: 27,609 bytes <!-- inventory:entry-cost --> —
  선행 로드 계약 파일들의 **바이트** 실측이다(bytes/3으로 대략 9k 토큰이지만 이는 **근사이지
  토큰 계수가 아니다**). 이 숫자는 SKILL.md 본문(약 9k 토큰)·스폰마다 붙는 에이전트 정의·런타임
  훅 주입·시점 로드를 **의도적으로 제외**한다 — 전체 컨텍스트 청구서가 아니라 한 차원의 하한으로
  읽어야 한다. 선행/시점 구분은 `<!-- always-read -->` 앵커가 선언하고, 참조 수와 바이트 크기가
  모두 ratchet에 걸려 있어 증가하려면 JUDGMENT 기록과 함께 baseline을 의식적으로 갱신해야 한다.
- **앱 1개당 토큰 비용**: 추정이 아니라 실측이다. 레퍼런스 파일럿(중간 규모 SPA를 intake부터 T0
  receipt까지)은 **약 12.7M 토큰 · 스폰 약 90회**를 썼고 그중 **약 11%가 재시도**였다. (파일럿
  종료 시점 기준 — telemetry 파일은 후속 작업으로 계속 누적되므로 살아있는 카운터가 아니라 기록된
  자릿수로 읽어야 한다. 정본은 `docs/efficacy/greenfield-pilot-2-protocol.md` 부록 A.) 이것이 정직한
  헤드라인이다 — 이 하네스는 **증거를 사는 것이고 증거는 비싸다**. 몇 분 만에 프로토타입이 필요하면
  프로토타입 도구를 쓰고, 이건 "다 된 것처럼 보인다"로 충분하지 않을 때 쓰는 물건이다.
