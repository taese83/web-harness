...
- **오케스트레이터 실행당 고정 계약 로드**: 46,908 bytes <!-- inventory:entry-cost --> —
...
첫 단어로 강제할 수 있다(플러그인 스킬은 항상 `web-harness:` 네임스페이스가 붙는다). 내부 스킬은
슬래시 메뉴에 뜨지 않고(`user-invocable: false`) 이 진입점이 읽는다 — 레인 표시와 게이트가 항상 걸린다.
...
- [docs/developer-guide.ko.md](docs/developer-guide.ko.md) — 티켓으로 개발할 때 하네스 쓰는 법(개발자 가이드)
